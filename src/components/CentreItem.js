...
const CentreItem = ({ center, id }) => {
  const navigation = useNavigation()
  return (
    <TouchableOpacity
      key={id}
      onPress={() => {
        navigation.navigate('Centre Details', { center })
      }}
    >
      <View style={styles.mainCard}>
        <View style={styles.imageCard}>
          <Image source={{ uri: center.image }} style={styles.img} />
        ...
        <View style={styles.mainContent}>
          <Text style={styles.mainTitle}>{center.name}</Text>
          ...
          >
            <View style={{ flexDirection: 'row' }}>
              <FontAwesome5 name="temperature-low" size={20} color="#2D1F21" />
